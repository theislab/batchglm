...
if __name__ == "__main__":
    ...
    linreg_trained = LinearRegression(X, y, fast=False)

    # define train function
    train_op = tf.train.AdamOptimizer(learning_rate=0.05)
    train_op = train_op.minimize(linreg_trained.squared_error, global_step=tf.train.get_global_step())

    initializer_op = tf.global_variables_initializer()

    sess.run(initializer_op)
    for i in range(1000):
        b_trained_res, loss_trained_res, _ = sess.run((linreg_trained.b, linreg_trained.squared_error, train_op))

    linreg_fast = LinearRegression(X, y)
    b_fast_res, loss_fast_res = sess.run((linreg_fast.b, linreg_fast.squared_error))
