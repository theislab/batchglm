...
class Model(BasicModel, metaclass=abc.ABCMeta):
    ...
    @property
    def sample_data(self) -> Union[np.ndarray, Iterable, int, float]:
    ...
    def count_probs(self, sample_data=None) -> Union[np.ndarray, Iterable, int, float]:
        if sample_data is None:
            sample_data = self.sample_data
        return rand_utils.NegativeBinomial(mean=self.mu, r=self.r).prob(sample_data)

    def log_count_probs(self, sample_data=None) -> Union[np.ndarray, Iterable, int, float]:
        if sample_data is None:
            sample_data = self.sample_data
        return rand_utils.NegativeBinomial(mean=self.mu, r=self.r).log_prob(sample_data)

    def log_likelihood(self, sample_data=None) -> Union[np.ndarray, Iterable, int, float]:
        if sample_data is None:
            sample_data = self.sample_data
        return np.sum(self.log_count_probs(sample_data))
