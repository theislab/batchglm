...
def closedform_beta_glm_logsamplesize(
    ...
    def apply_fun(grouping):
        ...
        # calculated variance via E(x)^2 or directly depending on whether `mean` was specified
        if provided_mean is None:
            ...
            if isinstance(X, SparseXArrayDataArray):
                Xdiff = X.add(- provided_mean, copy=True)
            else:
                Xdiff = grouped_data - provided_mean
        ...
        # TODO: ist gw_means das richtige? Funktioniert - 1?
        groupwise_scales = gw_means(1-gw_means)/variance - 1
    ...
    groupwise_scales, logsamplesize, rmsd, rank, _ = groupwise_solve_lm(
        dmat=design_scale,
        apply_fun=apply_fun,
        constraints=constraints
    )

    return groupwise_scales, logsamplesize, rmsd
