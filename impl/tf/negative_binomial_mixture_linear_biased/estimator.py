...
PARAMS = {
    "a": ("mixtures", "design_params", "genes"),
    "b": ("mixtures", "design_params", "genes"),
    "mu": ("samples", "genes"),
...
class Estimator(AbstractEstimator, TFEstimator, metaclass=abc.ABCMeta):
    ...
    def __init__(self, input_data: xr.Dataset,
        ...
        with model.graph.as_default():
            self.scaffold = tf.train.Scaffold(
                ...
                # summary_op=tf.group(model.merged_summary,
                #                     tf.py_func(self.save, [], [])),
    ...
    def _save_timestep(self, step, data: dict):
        ...
        xarray.to_netcdf(path=os.path.join(self.log_dir, "params-%04d.h5" % step))
