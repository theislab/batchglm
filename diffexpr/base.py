...
class DifferentialExpressionTest:
    ...
    def __init__(self, sample_data, full_model, reduced_model):
        self.sample_data = sample_data
        self.full_model = full_model
        self.reduced_model = reduced_model

    def likelihood_ratio_test(self):
        full = np.sum(self.full_model.log_count_probs(self.sample_data), axis=0)
        reduced = np.sum(self.reduced_model.log_count_probs(self.sample_data), axis=0)
...
def test(data, full_formula, reduced_formula, sample_description=None, model="negative_binomial", close_sessions=True):
    full_design = None
    reduced_design = None
    ...
    if model == "negative_binomial":
        import api.models.glm_nb as test_model

        logger.info("Estimating reduced model...")
        estim = test_model.Estimator(sample_data, design_matrix=reduced_design)
        estim.initialize()
        ...
        # a = np.zeros_like(full_design)
        # a[indices] = reduced_model.a
        # a_intercept = a[0]
        # a_slopes = a[1:]
        #
        # b = np.zeros_like(full_design)
        ...
        de_test = DifferentialExpressionTest(sample_data, full_model, reduced_model)
        return de_test
